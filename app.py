...
CHIAVE_SCRIPT = os.path.join(BASE_DIR, 'chiave.py')  # Script per generare la signatureM3U8_FILE = os.path.join(BASE_DIR, 'channels.m3u8')  # File m3u8 generato
...
def get_category_keywords():
    ...
    default_categories = {
        ...
        "RAI": ["rai"],
        "MEDIASET": ["mediaset", "canale 5", "rete 4", "italia", "focus", "tg com 24",
"premium crime", "iris"],
...
def create_manifest(mediaflow_url, mediaflow_psw):
    ...
    return {
        ...
        "name": "MediaFlow IPTV",
        "version": "1.0.0",
        "description": f"Watch IPTV channels from MediaFlow service ({mediaflow_url})",        "resources": ["catalog", "meta", "stream"],
        "types": ["tv"],
        "catalogs": catalogs,
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if os.path.exists(M3U8_FILE):
                ...
                print(f"Lista M3U8 generata con successo. Dimensione file: {file_size}
bytes")
                return True
            else:
...
async def catalog(type: str, id: str, request: Request, genre: str = None, search: str
= None):
    """Catalogo dei canali"""
...
if __name__ == "__main__":
    ...
    update_thread = threading.Thread(target=refresh_channels_periodically, daemon=True)    update_thread.start()
    # Avvia il server
