...
# Script per generare la lista m3u8
...
# Cache channels per non rigenerare la lista continuamentechannels_data_cache = []
...
def clean_channel_name(name):
    """Pulisce il nome del canale rimuovendo gli ultimi 3
caratteri se sono uno spazio, un punto e una lettera"""
    if len(name) > 3:
        last_three = name[-3:]
...
def extract_url_params(request: Request):
    ...
    try:
        ...
        if "/mfp/" in path and "/psw/" in path:
            ...
            if mfp_index < len(parts) - 1 and psw_index <
len(parts) - 1:
                mediaflow_url = unquote(parts[mfp_index +
1])
                mediaflow_psw = unquote(parts[psw_index +
1])
...
def get_channel_category(channel_name):
    """Determina la categoria di un canale in base al suo
nome"""
...
def generate_m3u8_list():
    ...
    try:
        ...
        # Controlla se lo script esiste prima di eseguirlo        if not os.path.exists(M3U8_GENERATOR):
        ...
        if result.returncode == 0:
            ...
            if os.path.exists(M3U8_FILE):
                ...
                print(f"Lista M3U8 generata con successo.
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith('#EXTINF:'):
                ...
                # Estrai il genere/categoria dal gruppo o
determinala dal nome
...
def get_channels_data():
    """Ottiene la lista dei canali, rigenerandola solo se
necessario"""
    ...
    if not channels_data_cache or (current_time - channels_data_timestamp) > 3600:  # 1 ora
        ...
        if not channels:
            print("File JSON vuoto o inesistente, analisi
del file M3U8...")
            channels = parse_m3u8_to_channels()
            print(f"Analizzati {len(channels)} canali da M3U8")
        # Se ancora non abbiamo canali, usa alcuni canali
di esempio
        if not channels:
            print("Nessun canale trovato, utilizzo canali
di esempio...")
            channels = [
                {"id": "rai1-example", "name": "Rai 1", "url": "https://example.com/rai1.m3u8", "genre": "RAI"},
                {"id": "canale5-example", "name": "Canale
5", "url": "https://example.com/canale5.m3u8", "genre": "MEDIASET"},
            ...
            if "localhost" not in stream_url:
                try:
                    ...
                    response = requests.post(
                        ...
                        timeout=10  # Aggiungi un timeout
per evitare blocchi
                    )
                    ...
                    print(f"Errore nella risoluzione URL:
{e}")
            ...
            final_url = f"https://{mediaflow_url}/proxy/hls/manifest.m3u8?{urlencode(params, quote_via=quote_plus)}"        else:
            ...
            for key, value in headers.items():
                params[f"h_{key}"] = value
            final_url = f"https://{mediaflow_url}/proxy/hls/manifest.m3u8?{urlencode(params, quote_via=quote_plus)}"    else:
        # URL normale senza signature, usa solo MediaFlow
con gli header standard
        params = {
            "api_password": mediaflow_psw,
            "d": stream_url
        }
...
def get_all_channels(mediaflow_url, mediaflow_psw):
    ...
    if not mediaflow_url or not mediaflow_psw:
        print("ERRORE: URL o password MediaFlow mancante")        return []
    try:
        ...
        print(f"ERRORE generico in get_all_channels: {e}")        return []
...
def refresh_channels_periodically():
    ...
    while True:
        ...
        try:
            ...
            print(f"Errore nell'aggiornamento dei canali:
{e}")
...
650px; margin: 0 auto; padding: 20px; }
        .header { text-align: center; margin-bottom: 30px; }
        .form-group { margin-bottom: 20px; }
        label { display: block; margin-bottom: 8px; font-weight: bold; }
        input[type="text"], input[type="password"] { width: 100%; padding: 12px; font-size: 16px; border: 1px solid
#ccc; border-radius: 4px; }
        .btn { display: inline-block; padding: 12px 20px;
background: #4caf50; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; }
        ...
        .url-display { word-break: break-all; padding: 10px; background: #eee; border-radius: 4px; margin: 15px 0; }    </style>
</head>
<body>
    <div class="header">
    ...
    <script>
        document.getElementById('generateLink').addEventListener('click', function() {
            ...
            const domain = '{{ domain }}';
            const encodedUrl = encodeURIComponent(mfpUrl);            const encodedPsw = encodeURIComponent(mfpPsw);
...
async def status():
    ...
    m3u8_size = os.path.getsize(M3U8_FILE) if m3u8_exists
else 0
    ...
    m3u8_generator_exists = os.path.exists(M3U8_GENERATOR)    chiave_script_exists = os.path.exists(CHIAVE_SCRIPT)
    return {
        "m3u8_file": {
            "exists": m3u8_exists,
...
async def meta_with_params(url: str, psw: str, type: str,
id: str):
    ...
    if type != "tv" or not id.startswith("mediaflow-"):
        return {"meta": {}}
    all_channels = get_all_channels(url, psw)
    channel = next((c for c in all_channels if c["id"] ==
id), None)
    if channel:
        return {"meta": channel}
    else:
...
if __name__ == "__main__":
    ...
    create_index_template()#!/usr/bin/env python3
import json
import os
import re
import time
...
if __name__ == "__main__":
    ...
    print("=== Inizializzazione addon MediaFlow IPTV ===")    print(f"Cartella corrente: {os.getcwd()}")
    ...
    # Genera la lista canali all'avvio
    print("Generazione lista canali all'avvio...")
    generate_m3u8_list()  # Prima genera la lista M3U8#!/usr/bin/env python3
import json
import os
import re
import time
...
if __name__ == "__main__":
    ...
    # Avvia un thread per l'aggiornamento periodico
    print("Avvio thread per aggiornamento periodico...")
    update_thread = threading.Thread(target=refr#!/usr/bin/env python3
import json
import os
import re
import time
...
if __name__ == "__main__":
    ...
    # Avvia il server
    print(f"Avvio server su porta {PORT}...")
    uvicorn.run(app, host="0.0.0.0", port=PORT)esh_channels_periodically, daemon=True)
    update_thread.start()
    # Avvia il server
    print(f"Avvio server su porta {PORT}...")
    uvicorn.run(app, host="0.0.0.0", port=PORT)#!/usr/bin/env python3
import json
import os
import re
import time
